...
public RSS feed - Mafindo's own IFCN-certified hoax debunking site.

The feed only exposes the ~10 most recent items with no working pagination, so
this is a small, freshness-biased corpus, not a historical archive. Safe and
idempotent to re-run periodically (skips items whose source_url is already
stored).

Only items labelled as an actual false/fraudulent claim are kept (title prefix
[SALAH]/[HOAX]/[PENIPUAN]/[DISINFORMASI]/[MANIPULASI]) - a wrongly-included true
statement would poison the corpus, since compute_falseness_score() treats every
row here as "similarity to this = high Falseness".

Embeds the English translation of the claim text, not the raw Indonesian title -
the embedding model is English-only. Falls back to embedding the raw Indonesian
text if the LLM call fails or no key is configured.
